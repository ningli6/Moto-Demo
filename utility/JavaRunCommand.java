...
public class JavaRunCommand {
    ...
    public static boolean sendEmail(String recv, String message, int nc) {
        if (recv == null || recv.length() == 0 || message == null || message.length() == 0) return false;
        String s = null;
        try {
            // using the Runtime exec method:
            Process p = Runtime.getRuntime().exec("python C:\\Users\\Administrator\\Desktop\\motoDemo\\python\\send.py " + recv + " " + Integer.toString(nc) + " " + message);
    ...
    public static boolean generatePlot(int nc) {
